...
class MockExerciseDataSource implements ExerciseDataSource {
  ...
  final List<Exercise> _exerciseList = [
    BodyWeightExercise(name: "Pull up", img: "pull_up.png", tracked: true, records: [
      BodyWeightExerciseRecord(reps: 15),
      BodyWeightExerciseRecord(reps: 13)
    ]),
    BodyWeightExercise(name: "Dip", img: "dip.png", tracked: true, records: [
      BodyWeightExerciseRecord(reps: 10),
      BodyWeightExerciseRecord(reps: 12)
    ]),
    BodyWeightExercise(name: "Pistol squat", img: "pistol_squat.png", tracked: false, records: []),
    WeightedExercise(name: "Bench press", img: "bench_press.png", tracked: true, records: [
  ...
  Future<Exercise?> getExerciseByName(String name) async {
    await Future.delayed(const Duration(milliseconds: 200));
    return _exerciseList.firstWhere(
            (el) => el.name == name
    );
  }
  ...
  Future<void> updateExercise(Exercise exercise) async {
    int i = _exerciseList.indexWhere((el) => el.name == exercise.name);
    _exerciseList[i] = exercise;
  }
